if (1) {
    fetch('https://me.guozy.online/api/memo?creatorId=1&tag=说说&limit=30').then(res => res.json()).then(data => { // 注意修改域名和用户id
        let items = [],
            html = '',
        ...
        items.forEach(item => {
            html += `<div class="talk_item"><div class="talk_meta"><img class="no-lightbox nolazyload avatar" src="https://cdn.guozy.online/img/2023/07/ea91e32f39f654117e3194ec61a508e9.jpeg"><div class="info"><span class="talk_nick">Aurora${icon}</span><span class="talk_date">${item.date}</span></div></div><div class="talk_content">${item.content}</div><div class="talk_bottom"><div><span class="talk_tag"># ${item.tag}</span></div><a href="javascript:;"onclick="rm.rightMenuCommentText('${item.text.trim()}')"><span class="icon"><i class="fa-solid fa-message fa-fw"></i></span></a></div></div>` // 注意修改头像链接和名称
        })
        document.getElementById('talk').innerHTML = html
        reflashEssayWaterFall()
    })
    // 监听窗口变化
    // 将 MutationObserver 实例绑定到一个目标节点
    const targetNode = document.getElementById("talk");
    // 创建一个 MutationObserver 实例，监听目标节点的变化
    const observer = new MutationObserver(reflashEssayWaterFall)
    ...
    function reflashEssayWaterFall() {
        const waterfallEl = document.getElementById("talk");
        if (waterfallEl) {
            setTimeout(function () {
    ...
    function Format(item) {
        let date = getTime(new Date(item.createdTs * 1000).toString()),
            ...
            music = content.match(/{\s*music\s*(.*)\s*}/g),
            text = ''
        ...
        let id, server;
        const playerMap = {
            "https://music.163.com": "netease",
            "https://y.qq.com": "tencent"
        }
        if (music) {
            music.forEach(mItem => {
                Object.keys(playerMap).forEach(item => {
                    if (new RegExp(item).test(mItem)) {
                        mItem = mItem.replace(/{\s*music\s*(.*)\s*}/, '$1').trim()
                        server = playerMap[item]
                        id = /[A-Za-z0-9]+$/.exec(mItem)
                    }
                })
                content += `<div class="bber-music"><meting-js id="${id[0]}" server="${server}" type="song" mutex="true" preload="none" theme="var(--anzhiyu-main)" data-lrctype="0" order="list"></meting-js></div>`
            })
        }
        return {
            content: content,
