...
function whenDOMReady() {
    if (location.pathname == '/dailyPhoto/') photos();
}
whenDOMReady()
...
// 自适应
window.onresize = () => {
    if (location.pathname == '/dailyPhoto/') waterfall('.gallery-photos');
};

// 函数 以下为23-06-10更新区域
function photos() {
    let url = 'https://me.guozy.online' // 修改api
    fetch(url + '/api/memo?creatorId=1&tag=相册').then(res => res.json()).then(data => {
        let html = '',
            imgs = []
