function sendJsonpRequest(url, data, callback) {
    ...
    script.src = url + '?callback=' + callbackName + '&' + queryParams
...
sendJsonpRequest(url, {
    key: 'QYCBZ-3RIWU-TGJV4-GCA5H-OTLZV-BTBKZ',
    output: 'jsonp',
}, function (res) {
    ipLocation = res
});
...
function showWelcome() {
    ...
    let dist = getDistance(116.36694, 40.08838, ipLocation.result.location.lng, ipLocation.result.location.lat); //这里记得换成自己的经纬度
    ...
    try {
        ...
        document.getElementById("welcome-info").innerHTML =
            `欢迎
            <b>
                <span style="color: var(--kouseki-ip-color);font-size: var(--kouseki-gl-size)">${pos}</span>
            </b> 的小友
            <br>
            ${posdesc}🍂
            <br>
                当前位置距博主约 
            <b>
                <span style="color: var(--kouseki-ip-color)">${dist}</span>
            </b> 
            公里！
            <br>
            ${timeChange} 
